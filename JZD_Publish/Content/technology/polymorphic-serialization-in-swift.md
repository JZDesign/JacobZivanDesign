...
Serialization is one of the most common things I have to handle in my role as a software engineer. It is pretty simple usually. The serialization I'm referring to is the mapping of Json into your model object in whatever language you're using. 
...
Weapons and armor are 2 different items that one could expect to see in many video games. They are dramatically different things, and as such, they don't appear the same way in the json. 
...
See the difference? When I was newer to the craft, my gut would have been to create a single Item model with every possible field represented and all of them would be optional. Something like:
...
To handle an API response that could give you several different models for a given key we'll use an enumeration.
...
Swift [Enumerations](https://docs.swift.org/swift-book/LanguageGuide/Enumerations.html) are quite powerful. One of my favorite features by far is the Associated Value. This is somewhat advanced. It allows us to define a set of values _(the enum)_ that are always the same, but then inject other variable values into the enumeration _(the associated value)_. It looks like this:
...
If you're used to Swift, you've seen this enumeration before. To use it, we have to inject and extract the value from the enumeration. Here are some examples on how that works:

```swift
func doSomething(with anotherThing: Int?) -> Result<Int> {
    guard let thing = anotherThing else {
...
Taking a look a the backpack json above, the `Item` we made before that was used in the `Inventory` struct wouldn't handle this well. Instead `Item` will need to be an enum with the kinds of items that could be represented inside.
...
This is looking better. However, if you try to serialize this, you'll get failures. The system doesn't know which enumeration value to select because these raw types like String or Int at the top level, they're nested within the enumeration. We need to provide a little more detail for Swift to encode or decode into this enum. To do so, we'll add the `Encodable` function and `Decodable` initializer.
...
enum Item: Codable {
    ...
    init (from decoder: Decoder) throws {
        if let weapon = try? Weapon(from: decoder) {
            self = .weapon(weapon)
        } else if let armor = Armor(from: decoder) {
            self = .armor(armor)
        } else {
    ...
    func encode(to encoder: Encoder) throws {
        switch self {
        case .armor(let armor): try armor.encode(to: encoder)
        case .weapon(let weapon): try weapon.encode(to: encoder)
        }
    }
}
```
